...
route_data |> 
  ...
  tm_shape() +
  tm_lines(col = "direction", col_alpha = 0.06)
}
```


# John Nolen Drive
Travel time estimates for the John Nolen Drive corridor between Rimrock and the "Hairball" (JND/Wilson/Willy/Blair). Note that for this route, the shortest duration route is shown even when that involves not taking JND itself but an alternative route. 
...
route_data <- travel_time |> 
  ...
  group_by(request_time_UTC) |> 
  mutate(shortest_duration = min(duration)) |> 
  filter(duration == shortest_duration)
```
...
```{r}
route_data <- travel_time |> 
  filter(route_id == "Johnson (Babcock/Orchard* to First)" & distance < 5020) 
```
